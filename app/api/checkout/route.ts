...
export async function POST(request: NextRequest) {
    try {
        ...
        if (!user.stripeOnboarded) {
            console.warn(
                `User ${user.id} creating payment link with incomplete Stripe onboarding`
            );
        }
        ...
        // Calculer le montant à transférer au client (92% du total)
        // Le client reçoit le montant total moins la commission de 8%
        const transferAmount = paymentLink.amount - platformFee;
        ...
        if (user.stripeOnboarded) {
            sessionConfig.payment_intent_data = {
                transfer_data: {
                    ...
                    amount: transferAmount, // Montant exact à transférer (92%)
                },
            };
        } else {
            console.warn(
                "Creating checkout without transfers - Stripe account not fully onboarded"
            );
        }
